...
export async function POST(req: Request) {
  ...
  if (ext !== 'pdf' && ext !== 'md') {
    return NextResponse.json({ error: 'Solo se aceptan archivos PDF o MD' }, { status: 400 })
  }
  ...
  try {
    ...
    return NextResponse.json({ error: 'No se pudo extraer el texto del archivo' }, { status: 422 })
  }
  ...
  if (chunkRows.length > 0) {
    ...
    if (chunksError) {
      ...
      return NextResponse.json({ error: 'Error al guardar los chunks' }, { status: 500 })
    }
  }

  const finalStatus = chunkRows.length === 0 ? 'error' : 'ready'
  await supabase.from('documents').update({ status: finalStatus }).eq('id', documentId)
