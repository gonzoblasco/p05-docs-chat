...
export async function POST(req: Request) {
  ...
  if (!query.trim()) {
    return NextResponse.json({ error: 'La query no puede estar vacía' }, { status: 400 })
  }
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      if (!chunks || chunks.length === 0) {
        send({ type: 'text', content: 'No encontré información relevante en los documentos seleccionados.' })
